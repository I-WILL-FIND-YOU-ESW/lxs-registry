...
| File | Used by |
|---|---|
| `prof-accountant.webp` | Accountant card |
| `prof-doctor.webp` | Doctor card |
| `prof-teacher.webp` | Teacher card |
| `prof-cafe.webp` | Café owner card |
| `prof-designer.webp` | Freelance designer card |
| `step-app.webp` | Step 2 header — "Your app" |
| `step-build.webp` | Step 3 header — "Your build" |
...
> Modular capability tiles snapping together into one glowing app: several
> rounded square tiles, each carrying a single simple icon (padlock, open book,
> picture, bell, person), connecting around a central glowing rounded app card,
> joined by thin lines like a small constellation / node graph — an app
> assembled from reusable parts.

**step-build.webp** — *Step 3 — "Your build": an agent builds it.*
> A friendly AI coding assistant: a small glowing spark / orb sidekick beside
> an open terminal window drawn as simple abstract lines with one blinking
> cursor block, a speech bubble with three dots, and a faint node-graph motif
> in the background — conveying "describe it and an agent builds it".
