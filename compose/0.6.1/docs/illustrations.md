...
The chosen profession's illustration **carries through** the wizard: the same
image shows on the "Your app" and "Your build" headers, so one artwork per role
covers the whole flow. No separate step images are needed. Until a role's file
exists, every slot for it shows the themed placeholder.
