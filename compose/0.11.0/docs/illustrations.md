...
| `prof-lawyer.webp` | Lawyer card, and steps 2/3 when Lawyer is chosen |
| `prof-trades.webp` | Home-service pro card, and steps 2/3 when chosen |
| `prof-salon.webp` | Salon owner card, and steps 2/3 when chosen |
| `prof-landlord.webp` | Landlord card, and steps 2/3 when chosen |
| `prof-organizer.webp` | Community organizer card, and steps 2/3 when chosen |
| `prof-student.webp` | Student card, and steps 2/3 when chosen |
...
**prof-lawyer.webp** — *The lawyer / client portal.*
> A calm legal desk: an open laptop showing an abstract document with a simple
> seal, a neat stack of case folders, reading glasses, a gavel resting on a law
> book, a small floating shield/lock icon for confidential client messages, and
> a tiny calendar icon for court dates.

**prof-trades.webp** — *The home-service pro / job portal.*
> A friendly tradesperson's scene: a toolbox and a cordless drill, a step
> ladder, a smartphone showing an abstract job card with a photo, a small job
> calendar, and a van key — practical, warm, no text.

**prof-salon.webp** — *The salon owner / booking page.*
> A cosy salon scene: a styling chair and mirror, scissors and a comb, a
> smartphone showing an abstract appointment calendar, a small swatch palette of
> hair tones, and a plant, with a tiny bell icon for reminders.

**prof-landlord.webp** — *The landlord / tenant portal.*
> A property scene: a small block of apartments, a keyring of keys, a folder of
> leases, a wrench icon for maintenance, and a phone showing an abstract
> notice/reminder card.

**prof-organizer.webp** — *The community organizer / community space.*
> A community scene: a few simple people figures around a table, a sign-up
> clipboard, a small banner/flag, a megaphone icon, and a phone showing an
> abstract event card.

**prof-student.webp** — *The student / study space.*
> A student's desk: an open laptop showing an abstract notes page, a stack of
> books with a bookmark, highlighters, a coffee cup, headphones, and a small
> floating calendar with a checkmark for deadlines.
...
## Tutorial screenshots (step 3)

Step 3 is a four-screen guided setup. Its images are **product mock-ups**, not
illustrations: a terminal, an agent window, and a browser. They are rendered
from `assets-src/tutorial-mocks.html` and captured with headless Chrome, so
they are reproducible and easy to re-skin:

```bash
# 1) screenshot each scene at 2x (CDP clip on #shot-install/#shot-open/#shot-prompt/#shot-run)
# 2) resize to 1600 wide WebP
cwebp -q 82 -resize 1600 0 /tmp/shot-install.png -o backend/static/img/step-install.webp
# …step-open.webp, step-prompt.webp, step-run.webp
```

| File | Screen |
|---|---|
| `step-install.webp` | 1 — install the eco CLI (terminal) |
| `step-open.webp` | 2 — open the project in an agent |
| `step-prompt.webp` | 3 — paste the prompt (agent building) |
| `step-run.webp` | 4 — run & publish (the app running) |
